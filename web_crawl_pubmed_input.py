...
if __name__ == "__main__":
    indir = str(input('Directory for .xlsx, .xls or .csv files: '))
    outdir = str(input('Directory to save the output file: '))
    outname = str(input('Prefix of the output file name: '))
    abstract_filter = str(input('Select the articles that contain the given word: '))
    ...
    for read_file in read_file_list:
        ...
        # Filter the results
        if abstract_filter.strip()!="": df = df[df['초록(영문)'].str.contains(abstract_filter)]
        # Save the results
